...
impl Downstream {
    ...
    fn handle_authorize_token(&self, new_token: &str) -> AuthorizeTokenAction {
        self.shared_token_state
            .safe_lock(|state| state.handle_authorize_token(new_token))
            .unwrap_or_else(|e| {
                error!("Failed to lock shared token state: {:?}", e);
                ProxyState::update_inconsistency(Some(1));
                let current_token = self.token.safe_lock(|t| t.clone()).unwrap_or_default();
                AuthorizeTokenAction::KeepCurrent(current_token)
            })
    }
...
mod tests {
    ...
    async fn unchanged_token_does_not_increment_shared_counter_and_authorize_is_deduplicated() {
        ...
        let second = downstream
            .safe_lock(|d| {
                let action = d.handle_authorize_token("");
                d.authorize("worker-b");
                d.authorize("worker-b");
                action
            })
            .unwrap();
        let (authorized_names, token) = downstream
            .safe_lock(|d| {
                (
