...
impl MultiUpstreamManager {
    ...
    async fn handle_upstream_messages(
        ...
        while let Some(message) = receiver.recv().await {
            if let Err(e) = sender.send(message).await {
                error!("Failed to forward message from upstream {}: {}", upstream_id, e);
                break;
            }
        }
    ...
    async fn connect_upstream_with_retry(
        ...
        while retry_count < MAX_RETRIES {
            info!("Connecting to upstream {} (attempt {}/{}): {}", id, retry_count + 1, MAX_RETRIES, address);
            ...
            match crate::minin_pool_connection::connect_pool(
                ...
                Err(e) => {
                    error!("Failed to connect to upstream {} (attempt {}): {}", id, retry_count + 1, e);
                    retry_count += 1;
        ...
        error!("Failed to connect to upstream {} after {} attempts", id, MAX_RETRIES);
   ...
   pub async fn broadcast(&self, message: PoolExtMessages<'static>) -> Vec<String> {
        ...
        let active_upstreams: Vec<_> = upstreams.values()
        ...
        for upstream in active_upstreams {
            match upstream.sender.send(message.clone()).await {
                ...
                Err(e) => {
                    ...
                    warn!("Upstream {} send failed, may need reconnection: {}", upstream.id, e);
                }
            }
        }
